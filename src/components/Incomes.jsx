...
import { getIncomeCategories } from './AddTransactionModal';
import { useTranslation } from '../utils/LanguageContext';

const Incomes = ({ transactions, settings, onAddClick, onDeleteTransaction }) => {
  ...
  const filteredIncomes = incomes.filter((tCode) => {
  ...
  const incomeByCategory = incomes.reduce((acc, tCode) => {
    acc[tCode.category] = (acc[tCode.category] || 0) + tCode.amount;
    return acc;
  }, {});

  const categoryBreakdown = incomeCategories.map((cat) => {
  ...
  const getCategoryDetails = (catId) => {
    return incomeCategories.find((c) => c.id === catId) || { name: t('cat_other_income'), color: '#6B7280' };
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-extrabold text-white tracking-tight">{t('incTitle')}</h1>
        ...
        <button
          onClick={onAddClick}
          className="w-full md:w-auto px-5 py-3 bg-emerald-500 hover:bg-emerald-600 text-white font-bold rounded-2xl shadow-lg shadow-emerald-500/20 hover:shadow-emerald-500/30 transition-all duration-300 transform active:scale-95 flex items-center justify-center gap-2 cursor-pointer"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl relative overflow-hidden">
          ...
          <h3 className="text-2xl font-black text-emerald-400 mt-2">{formatMoney(totalIncome)}</h3>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl relative overflow-hidden">
          ...
          <h3 className="text-2xl font-black text-white mt-2">{formatMoney(averageIncome)}</h3>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl relative overflow-hidden">
          ...
          <h3 className="text-2xl font-black text-white mt-2">{formatMoney(maxIncome)}</h3>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button
                        ...
                        className="p-1.5 text-slate-650 hover:text-rose-500 bg-transparent hover:bg-rose-500/10 rounded-xl opacity-0 group-hover:opacity-100 transition-all duration-200 cursor-pointer"
                        title="Удалить"
                      >
                        🗑️
                      </button>
        ...
        {/* Right Side: Category Breakdown Chart */}
                ...
                <div key={cat.id} className="space-y-1.5">
                  ...
                  <div className="w-full h-2 bg-slate-950 rounded-full overflow-hidden p-0.5 border border-slate-900">
                    <div
                      ...
                      className="h-full rounded-full shadow-sm shadow-emerald-500/20"
                    ></div>
                  </div>
                </div>
              ))}
